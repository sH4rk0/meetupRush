...
(function (MUR) {
    var GameState = (function (_super) {
        ...
        function GameState() {
            ...
            _this.readyOnceEnd = true;
            _this.currentPlayerExist = false;
            _this.goal = settings.goalDistance;
            return _this;
        }
        ...
        return GameState;
    }(Phaser.State)); //end class
    MUR.GameState = GameState;
})(MUR || (MUR = {})); //end module
/// <reference path="../Lib/phaser.d.ts"/>
var MUR;
(function (MUR) {
...
(function (MUR) {
    ...
    })(PlayerStates = MUR.PlayerStates || (MUR.PlayerStates = {}));
    var Player = (function (_super) {
        __extends(Player, _super);
        function Player(game, gameState, id, name, isPlayer, startX, startY, avatar) {
            var _this = _super.call(this, game, startX, startY, "players") || this;
            _this.game = game;
            _this.id = id;
            _this.name = name;
            ...
            _this.lastRate = 0;
            _this.playerSpeed = 0;
            _this.anim = new Array;
            _this.autoRun = MUR.getUrlParameter("autoRun") ? true : false;
            _this.scale.set(2);
            ...
            anim = _this.animations.add("run", frames[avatar][0], 0, true);
            _this.anim.push(anim);
            _this.anim[PlayerStates.IDLE].play();
            _this.currentState = PlayerStates.IDLE;
            if (_this.isPlayer) {
                _this.alpha = 1;
                ...
                if (_this.game.device.touch && (_this.game.device.iOS || _this.game.device.android || _this.game.device.windowsPhone)) {
                    _this.gameState.introRoad.inputEnabled = true;
                    _this.gameState.introRoad.events.onInputDown.add(_this.tapInput, _this);
                }
                else {
                    _this.cursors = _this.game.input.keyboard.createCursorKeys();
                    _this.leftButton = _this.game.input.keyboard.addKey(Phaser.Keyboard.LEFT);
                    _this.leftButton.onDown.add(_this.arrowLeft, _this);
                    _this.rightButton = _this.game.input.keyboard.addKey(Phaser.Keyboard.RIGHT);
                    _this.rightButton.onDown.add(_this.arrowRight, _this);
                }
            }
            else {
                _this.alpha = 0.3;
